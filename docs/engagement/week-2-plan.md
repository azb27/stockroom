...
| 2. No missing day reported as zero | **Met** | 9 outage questions, 36 answers across 4 runs, all "unknown" |
| 3. Buyer approves or edits every PO | **Met by design** | No tool can approve or send. Approval is a named human action (CLI or UI). |
| 4. Forecast beats the same-weekday method | **Met at SKU level; a tie on slow sellers** | WAPE 73.2% vs 76.8%, CI on the gain [+3.3, +4.0] pp; slow sellers 116.7% vs 115.9% (`docs/results/forecast_backtest.md`) |
...
| 4 | **Put metric definitions in tools.** Add a zero-filled daily sales view, and send "rate" and "days of cover" questions to tools that compute them. | Closes the one open failure (wrong denominator, `eval_failure_analysis.md` #2) | FDE | The failing question passes, and a new eval question guards it |
