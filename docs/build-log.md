# Build log: how Stockroom was built, phase by phase

*The README is the product view; this is the engineering diary. Each phase's numbers come from generated result files, linked inline. The plan of record is [`SPEC.md`](../SPEC.md); decisions are in [`adr/`](adr).*
...
| `detect_anomalies` | Stock-outs, stock-out risk within lead time, overstock, sales spikes/drops | All 498 planted stock-outs with recent demand found; 99.7% of overstock flags correct |
...
1. A parser allowlist: one SELECT, cleaned tables only, no table functions. It rejects 27 hostile or malformed queries, including `query('...')` smuggling, `read_csv` of a local file and attaching or reading the ground-truth file.
...
- **No look-ahead, proven:** a test deletes every sale and price after the backtest origin and checks that no feature changes. (Checked by hand during P3: shortening the look-back by one day makes that test fail.)
- **Reproducible:** training reads rows in a fixed order (tested), and two runs by hand during P3 produced identical results.
...
**Live check, 5/5 correct** ([transcript](results/agent_session_p4.md), generated), for $0.10 total:
...
**What the cleaning layer is worth:** on lookups and aggregations, the same model scores 100% with it and **60%** without it. Across all 120 questions, 44 are right only with cleaning and 0 the other way (p ≈ 10⁻¹³). On raw data it scored 0 of 6 on monthly revenue, 0 of 2 on the twice-loaded week and 0 of 3 on re-coded SKUs. In one trace it *noticed* a price keyed in cents and reported it anyway ([failure analysis](results/eval_failure_analysis.md)).

**The eval found a product bug.** Agents filtered `status = 'active'` (the data says `'ACTIVE'`), got nothing back, and confidently answered 0. `run_sql` now names the mismatch when a result is empty. Haiku went from 92% to 96% overall (p = 0.18 with one run per configuration: the right direction, not yet proven), and from 67% to 87% on multi-step questions.
...
CI runs lint, the data build and the tests (those needing a trained forecast skip) on every push to main and every PR. PRs that touch `src/` or `evals/` also run a 20-question Haiku smoke eval (when an API key secret is set) and fails if accuracy drops more than 10 points below [`evals/baseline.json`](../evals/baseline.json).

## Phase 6: the same tools in Claude Code and Claude Desktop
`stockroom-mcp` serves the six tools over MCP: stdio for desktop clients, and streamable HTTP on 127.0.0.1. It is a thin adapter over the same registry the eval measured ([ADR 0006](adr/0006-mcp-server-registry-driven-loopback-only.md)):
...
**Live check, 8/8 correct** ([transcript](results/claude_code_session_p6.md), generated), $0.41 total. Eight eval questions were sent through headless Claude Code (`claude -p`) with its built-in file and shell tools switched off, so every answer had to come through the MCP server:
...
![Stockroom web demo](images/web_demo.png)

A chat with a live **tool trace** and a **PO approval queue**, in one container ([ADR 0007](adr/0007-one-container-demo-on-hf-spaces.md)):
